...
typealias Ranges = List<Triple<String, String?, Visitor?>>

data class Range(val lines: List<String>, val part: String?, val form: Visitor?)
...
val forms = mutableMapOf<File, Visitor>()
...
class Visitor(file: File, val scope: CoroutineScope) : FormBaseVisitor<Any?>() {
  ...
  val completeForms = mutableListOf<Visitor>()
  ...
  fun FormParser.InstructionContext.ifConditionOrDefault(
    ...
    val bound = condition()
    return this@Visitor.ifConditionOrDefault(default) {
  ...
  fun FormParser.OnClauseContext.assign(condition: Deferred<Boolean>, consequent: Deferred<Any?>?) {
    for (ranges in ranges()) {
      for ((line, part, form) in
        ...
        assert(part == null)
        (form ?: this@Visitor).assigns.getOrPut(line) { mutableListOf() } += condition to consequent
      }
    }
  }
  ...
  override fun visitCompleteForm(ctx: FormParser.CompleteFormContext): Job? {
    val form = visit(ctx.ref()) as Visitor?
    completeFormsLock.lock()
    try {
  ...
  fun dependsOn(other: Visitor): Boolean =
  ...
  fun Iterable<Visitor?>.get(line: String, part: String? = null): Deferred<Any?> {
    val bound = mapNotNull { it?.get(line, part, this@Visitor) }
    return scope.async {
      @OptIn(ExperimentalCoroutinesApi::class)
  ...
  fun get(line: String, part: String?, caller: Visitor): Deferred<Any?>? {
  ...
  override fun visitRange(ctx: FormParser.RangeContext): Range {
    ...
    val (part, form) =
        ...
        else @Suppress("UNCHECKED_CAST") (visit(ofClause) as Pair<String?, Visitor?>)
    return Range(lines, part, form)
  }

  override fun visitOfClause(ctx: FormParser.OfClauseContext): Pair<String?, Visitor?> =
      ctx.part()?.id?.text?.normalize() to
          ctx.ref()?.let { visit(it) as Visitor? ?: throw IllegalExprException() }

  override fun visitRef(ctx: FormParser.RefContext): Visitor? {
...
fun CoroutineScope.evalForm(file: File): Visitor? {
  formsLock.lock()
  try {
    return forms.getOrElse(file) { if (!file.exists()) null else Visitor(file, this) }
  } finally {
    formsLock.unlock()
  }
}

typealias Forms = Map<File, Visitor>

typealias Overrides = Map<File, Map<String, Any?>>

fun Visitor.unusedRrspContributions(): BigDecimal =
    @OptIn(ExperimentalCoroutinesApi::class)
...
fun Visitor.choices(): List<Overrides> =
    mutableListOf<Overrides>().also { choices ->
      spousesReturn?.also {
        choices +=
            mapOf(
                directory to
                    mapOf("medical expenses for self" to spousesReturn.medicalExpensesForSelf),
                spousesReturn.directory to
                    mapOf("medical expenses for self" to medicalExpensesForSelf),
            )
      }
