    ...
    if (unusedRrspContributions > 0.toBigDecimal() &&
      ...
      forms.entries.removeAll { (otherFile, otherForm) ->
        otherForm.year > form.year &&
            ...
            """5000-s7-[0-9]{2}e\.txt""".toRegex().matches(otherFile.name)
      }
    }
  }
}
...
suspend fun Forms.minimizeBalance(choices: List<Overrides>): Forms {
  mapValues { (file) -> coroutineScope { evalForm(file)!! } }.deductRrspContributions()
  return (1..choices.size).flatMap { choices.combinations(it) }.fold(this) { bestForms, combination
    ->
    ...
    forms -= keys
    mapValues { (file) -> coroutineScope { evalForm(file)!! } }.deductRrspContributions()
    val newForms = mapValues { (file) -> coroutineScope { evalForm(file)!! } }
    for (choice in combination) {
      for ((file, fileOverrides) in choice) {
...
suspend fun main(args: Array<String>) {
  ...
  val year = directory.parent.toInt()
  val basename = "5010-r-%02de.txt".format(year % 100)
  coroutineScope { evalForm(File(directory, basename)) }
  for ((_, group) in
